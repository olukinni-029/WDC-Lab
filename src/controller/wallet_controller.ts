...
import {
    encryptPayloadToSingleField,
    getPartnerWithKey,
} from "../utils/common/encryptor";
...
export const WalletController = {
    ...
    signUpFee: asyncHandler(async (req: Request, res: Response) => {
        const url =
            (process.env.SUPPLY_BASE as string) + "partners/dynamic/account";
        ...
        const headers = {
            ...
            "merchant-id": req.headers["merchant-id"],
        }
    ...
    transfer: asyncHandler(async (req: Request, res: Response) => {
        ...
        const wallet = await WalletService.findByAccountNumber(
            ...
            WalletTransactionService.create({
                ...
                bankResponse: {
                    ...
                    originatorBankVerificationNumber:
                        result.originatorBankVerificationNumber,
    ...
    getVirtualWallets: asyncHandler(async (req: Request, res: Response) => {
        const { page, limit, accountNumber } = req.query;
        ...
        return successResponse(res, { result: result.items, pagedInfo }, "success");
    }),

    getAllUserWalletHistory: asyncHandler(async (req: Request, res: Response) => {
        const acccountNumber = req.body.accountNumber;
        if (!acccountNumber) return errorResponse(res, "account number required", 400);
    ...
    getAllUserTransactions: asyncHandler(async (req: Request, res: Response) => {
        const acccountNumber = req.body.accountNumber;
        if (!acccountNumber) return errorResponse(res, "accountNumber required", 400)
        const wallet = await WalletService.findByAccountNumber(acccountNumber);
    ...
    encryptData: asyncHandler(async (req: Request, res: Response) => {
        ...
        if (!user) return;
        const response = encryptPayloadToSingleField(
            req.body,
